...
from mymodel import BertForSequenceClassification, BertForCLSMLM

from tokenizer import BertTokenizer
from optimizer import AdamW, get_linear_schedule_with_warmup
from data_utils import processors, output_modes,  compute_metrics, convert_examples_to_features

from mlm_utils import load_and_cache_mlm_examples, pad_sequence, mask_tokens
...
def train(args, train_dataset, mlm_train_dataset, model, tokenizer):
    ...
    mlm_train_sampler = RandomSampler(mlm_train_dataset) if args.local_rank == -1 else DistributedSampler(mlm_train_dataset)
    mlm_train_dataloader = DataLoader(
        mlm_train_dataset, sampler=mlm_train_sampler, batch_size=args.train_batch_size, collate_fn=collate
    )
    ...
    best_dev_acc=0
    saved_model=False
    mlm_epoch_iterator = tqdm(mlm_train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0]).__iter__()
    mlm_step = 0
    for _ in train_iterator:
        ...
        #########################################  Your Code  ###########################################
        # todo
        # get batch_mlm and batch by iterating the epoch_iterator and mlm_epoch_iterator
        for step, batch in enumerate(epoch_iterator):
            if mlm_step >= len(mlm_train_dataloader):
                mlm_epoch_iterator = tqdm(mlm_train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0]).__iter__()
                mlm_step = 0
            batch_mlm = next(mlm_epoch_iterator)
            mlm_step += 1 
        #################################################################################################
            ...
            mlm_inputs, mlm_labels = mask_tokens(batch_mlm, tokenizer, args) 
            mlm_attention_mask = mlm_inputs.ne(tokenizer.pad_token_id).long()
            mlm_inputs = mlm_inputs.to(args.device)
            mlm_attention_mask = mlm_attention_mask.to(args.device)
            mlm_labels = mlm_labels.to(args.device)
            #################################################################################################
            
            outputs = model(input_ids=batch[0], attention_mask=batch[1], labels=batch[3], mlm_input_ids=mlm_inputs,mlm_attention_mask=mlm_attention_mask,mlm_labels=mlm_labels)
...
def main():
    ...
    parser.add_argument(
        "--small_labeled_data", action="store_true", help="Set this flag if you cut the number of the labeled training data to 50.",
    )
    ...
    model = BertForCLSMLM(model_name_or_path=args.model_name_or_path,num_labels=num_labels)
    ...
    if args.do_train:
        ...
        mlm_train_dataset = load_and_cache_mlm_examples(args, tokenizer, evaluate=False)
        global_step, tr_loss = train(args, train_dataset, mlm_train_dataset, model, tokenizer)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        model = BertForCLSMLM(args.model_name_or_path,num_labels)
        model.load_state_dict(saved['model'])
        model.to(args.device)
        result = evaluate(args, model, tokenizer, data_type='dev')
        result = evaluate(args, model, tokenizer, data_type='test')
...
'''
CUDA_VISIBLE_DEVICES=3 python classifier_mlm.py \
    --model_type bert \
    --model_name_or_path huawei-noah/TinyBERT_General_4L_312D \
    --task_name yelp \
    --do_train \
    --do_eval \
    --data_dir /home/zhoujing/NLP/data/classification/yelp_full/yelp_review_full_csv/zj_data/yelp_small \
    ...
    --max_seq_length 128 \
    --mlm_alpha 1.0 \
    --output_dir tmp/tinybert_unlabel_full \
    --evaluate_during_training \
    --train_data_file /home/zhoujing/NLP/data/classification/yelp_full/yelp_review_full_csv/zj_data/yelp_small/unlabeled_train_5000.txt \
    --block_size 128 \
    --line_by_line \
    --mlm_alpha 1.0 \
