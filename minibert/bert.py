...
# def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    """
    input: [matrix] the input
    output: [matrix] the input that requires the sublayer to transform
    dense_layer, dropput: [function] the sublayer
    ln_layer: [function] layer norm that takes input+sublayer(output)
    """
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    intermediate_input=self.add_norm(hidden_states,attention_output,self.attention_dense,self.attention_dropout,self.attention_layer_norm)
    # feed forward
    intermediate_output=self.interm_af(self.interm_dense(intermediate_input))
    # another add-norm layer
    outputs=self.add_norm(intermediate_input,intermediate_output,self.out_dense,self.out_dropout,self.out_layer_norm)
    return outputs
